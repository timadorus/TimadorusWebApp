...
public class CampaignPanel extends FormPanel implements HistoryListener {
	...
	public CampaignPanel(TimadorusWebApp _entry) {
		super();
		this.entry = _entry;
	...
	private void createUI() {
		...
		grid.setWidget(2, 2, new Label("Kampagnenname"));
		grid.setWidget(2, 4, tb_charname);
		grid.setWidget(3, 2, new Label("Geschlechter"));
		...
		cb_fraction.addChangeHandler(new ChangeHandler() {
			...
			public void onChange(ChangeEvent event) {
				...
				if (lb.getValue(lb.getSelectedIndex()).toLowerCase().equals(
					...
					System.out.println("gew�hltes Element: "
