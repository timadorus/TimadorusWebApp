	...
	public RegisterPanel(TimadorusWebApp timadorusWebApp) {
		...
		class MyHandler implements ClickHandler, KeyUpHandler {
			...
			private void handleEvent() {
				...
				User register = new User(vornameTextBox.getText(),
						nachnameTextBox.getText(), geburtstagTextBox.getText(),
						emailTextBox.getText(), usernameTextBox.getText(),
				...
				if (!register.isValid()) {
					...
					if (register.getVorname().length() == 0
							&& register.getNachname().length() == 0) {
					...
					History.newItem("register");
						...
						emailRepeatTextBox.getText())) {
					registerInvalid(User.EMAIL_FAULT);
				} else if (!passwordTextBox.getText().equals(
						passwordRepeatTextBox.getText())) {
			...
			private void sendToServer(User register) {
				RegisterServiceAsync registerServiceAsync = GWT
