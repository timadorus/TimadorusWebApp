...
public class CharacterPanel extends FormPanel implements HistoryListener {
  ...
  private void createUI() {
    ...
    cb_fraction.addChangeHandler(new ChangeHandler() {
      ...
      public void onChange(ChangeEvent event) {
        ...
        System.out.println(lb.getValue(lb.getSelectedIndex()));
        if (lb.getValue(lb.getSelectedIndex()).toLowerCase().equals("fraction 1")) {

          System.out.println("gew�hltes Element: " + lb.getValue(lb.getSelectedIndex()));

          for (String races : listeRace_1) {
            // System.out.println(races);
            cb_race.addItem(races);
          }
        } else if (lb.getValue(lb.getSelectedIndex()).toLowerCase().equals("fraction 2")) {
          System.out.println("gew�hltes Element: " + lb.getValue(lb.getSelectedIndex()));
        ...
        } else if (lb.getValue(lb.getSelectedIndex()).toLowerCase().equals("fraction 3")) {
          for (String races : listeRace_3) {
            // System.out.println(races);
            cb_race.addItem(races);
          }

        } else if (lb.getValue(lb.getSelectedIndex()).toLowerCase().equals("fraction 4")) {
    ...
    cb_race.addChangeHandler(new ChangeHandler() {
      ...
      public void onChange(ChangeEvent event) {
        ...
        if (lb.getValue(lb.getSelectedIndex()).toLowerCase().equals("race 1")) {
        ...
        } else if (lb.getValue(lb.getSelectedIndex()).toLowerCase().equals("race 2")) {
          System.out.println("gew�hltes Element: " + lb.getValue(lb.getSelectedIndex()));
        ...
        } else if (lb.getValue(lb.getSelectedIndex()).toLowerCase().equals("race 3")) {
  ...
  class MyHandler implements ClickHandler, KeyUpHandler {
    ...
    public void onClick(ClickEvent event) {
      ...
      // pr�feNamen(tb_charname.getText()));
  ...
  /**
   * Namen auf Gültigkeit prüfen Name des Characters. Zulässig sind die große und kleine Buchstaben, sowie Bindestrich,
   * Leerzeichen und Single-Quote (')
   * 
   * @param namen
   * @return
   */
  ...
  private void createvisibleDialog() {
    ...
    if (cb_gender.getSelectedIndex() != -1) {
      sb.append(cb_gender.getItemText(cb_gender.getSelectedIndex())).append("\n");
    } else {
      sb.append("\nfalscher index von cb_gender");
    }

    if (cb_fraction.getSelectedIndex() != -1) {
      sb.append(cb_fraction.getItemText(cb_fraction.getSelectedIndex())).append("\n");
    } else {
    ...
    if (cb_profession.getSelectedIndex() != -1) {
      sb.append(cb_profession.getItemText(cb_profession.getSelectedIndex())).append("\n");
    } else {
